...
export default function Navbar({themeFunc, mounted, resolvedTheme}: ThemeController): JSX.Element{
    return (
        <div className="bg-white dark:bg-black">
        <h1 className="text-black dark:text-white">{mounted ? 'Dark mode active' : 'Dark mode close'}</h1>
        <button className="p-2" onClick={() => themeFunc(resolvedTheme === "dark" ? "light" : "dark")}>
          {resolvedTheme === "dark" &&
          (<svg role="img" className="fill-current w-7 h-7" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-labelledby="color-scheme-light">
